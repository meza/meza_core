    ...
    private static void renderSupporters(ConfigCategory supportCategory, ConfigEntryBuilder entryBuilder) {
        ...
        if (!newSupportersText.getString().isBlank()) {
            supportCategory.addEntry(entryBuilder
                    .startTextDescription(Text.translatable("meza_core.config.support.newcomers"))
                    .build());
        ...
        if (!tiers.isEmpty()) {
            supportCategory.addEntry(entryBuilder.startTextDescription(Text.translatable("meza_core.config.support.list")).build());
        }
        ...
        if (tiers.isEmpty()) {
            supportCategory.addEntry(entryBuilder.startTextDescription(Text.translatable("meza_core.config.support.empty")).build());
        }
    }

    public static void add(ConfigBuilder builder, ConfigEntryBuilder entryBuilder) {
        Text description =Text.translatable("meza_core.config.support.description1")
                .append("\n")
                .append(Text.translatable("meza_core.config.support.description2"));

        /*? if >=1.21 {*/
        Text category = Text.literal("❤ ").append(Text.translatable("meza_core.config.category.support"));
        /*?} else {*/
        /*Text category = Text.translatable("meza_core.config.category.support");
        *//*?}*/
        ...
        ConfigCategory supportCategory = builder.getOrCreateCategory(category)
                ...
                .addEntry(new HeartTextEntry(Text.translatable("meza_core.config.support.cta")
                        .styled(style -> style
